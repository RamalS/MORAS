...
Proof.
  intros. destruct H, H0. simpl. rewrite H, H0.
  
  assert(ISn: forall (n : nat), I :: repeat I n = repeat I (S n)).
  {
    intros. simpl. reflexivity.
  }
  ...
  assert(AddLrIn: forall (n : nat), AddLr (repeat I n) (repeat I n) I = repeat I n).
  {
  ...
  assert(AddLIn: AddL (repeat I n) (repeat I n) = repeat I (pred n) ++ [O]).
  {
    intros. induction n.
    - simpl. contradiction.
    - simpl. rewrite ISn. unfold AddL. rewrite P_rev_2. simpl. rewrite AddLrIn. rewrite P_rev_2.
      reflexivity.
  }

  assert(NotLrep: forall (n : nat), NotL (repeat I n ++ [O]) = repeat O n ++ [I]).
  {
    intros. induction n0.
    - simpl. reflexivity.
    - simpl. rewrite IHn0. reflexivity.
  ...
  induction n.
  - contradiction.
  - simpl. rewrite ISn. rewrite AddLIn. simpl. rewrite NotLrep. reflexivity.
Qed.